...
async def create_user(user: UserCreate, session: Session = Depends(get_session_dependency)):
...
async def get_user(session: Session = Depends(get_session_dependency)):
    return crud.get_users(session)


@router.delete("/{user_id}/", description="Удалить пользователя")
async def delete_user(user_id: int, session: Session = Depends(get_session_dependency)):
    user = crud.get_user(session, user_id)
    if not user:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
...
async def update_user(user_id: int, user_update: UserUpdate, session: Session = Depends(get_session_dependency)):
    user = crud.get_user(session, user_id)
